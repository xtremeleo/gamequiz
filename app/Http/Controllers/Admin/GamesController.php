...
class GamesController extends Controller
{
	...
	public function get_quiz_fields()
	{
		...
		$fields = array(
							array("name" => "entry_nos", "type" => "number","display_name" => "Entry allowed", "value" => "", "required" => "required", "valid" => "required|string"),
	...
	public function action(Request $request)
	{
		if ($request->action == "add-quiz")
		{
			...
			else
			{
				...
				$question_nums = $request->questions;
				$questions = Question::inRandomOrder()->take($question_nums)->get();
				$question_ids = array_column($questions->toArray(), 'id');
				
				$new_quiz = new Quiz;
				$new_quiz->entry_nos = $request->entry_nos;
				$new_quiz->start_datetime = $start_datetime;
				$new_quiz->end_datetime = $end_datetime;
				$new_quiz->prize = $request->prize;
				$new_quiz->questions = json_encode($question_ids);
				$new_quiz->save();
